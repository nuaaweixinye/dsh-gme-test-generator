...
后端另有文档可查：`docs/backend-overview.md` 是它的功能与契约总览（任务生命周期、动作前置条件、"什么算通过"的校验规则、配置参考）；`docs/knowledge-injection.md` 讲它**默认关闭**的可选能力——生成前注入历史分歧与知识库参照。该能力在后端的 `config.local.json` 里开关，与本插件配置无关；唯一交点是托管进程的环境（见配置文档第 6 节）。
